...
1. **Docker Hub** — secrets `DOCKERHUB_USERNAME` (Hub username, e.g.
   `yauhenbichel`, not email) and `DOCKERHUB_TOKEN` (Access Token with
   Read & Write; no trailing newline). The **Release** workflow’s
